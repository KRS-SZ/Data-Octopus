...
class Toolbar1:
    """
    Obere Toolbar - VEREINFACHT: Nur Group + Param + Refresh + Custom Test + Save Data.

    Layout:
    [Group ▼] [Param ▼ (breit)] [Refresh] | [Custom Test] [Save Data] [Status]
    """
    ...
    def build(self) -> None:
        ...
        with ui.row().classes('w-full items-center gap-2 px-2 py-1 bg-gray-100 border-b'):
            ...
            ui.label('Param:').classes('text-sm font-bold')
            ...
            # Refresh Button
            ui.button('🔄 Refresh', on_click=self._handle_refresh).classes('bg-blue-500 text-white').props('dense')
